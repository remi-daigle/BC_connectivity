...
for(sp in unique(OBISusable$species)){
    index <- which(names(conmat) %in% OBISusable$gridID[OBISusable$species==sp])
    ...
    conmat[index,index] <- tempmat/max(tempmat)
    # conmat[index,index] <- conmat[index,index]+OBISusable$abundance[OBISusable$species==sp]%*%t(OBISusable$abundance[OBISusable$species==sp])
...
# conmat_bp <- read.csv("data/con_mat/con_mat_per_G1998_pld_8.csv",
#                                       row.names=paste0("gb",1:nrow(grid_boxes)),
#                                       col.names=paste0("gb",0:nrow(grid_boxes))
#                                       )[,-1]
# 
# ### infomap ###
# con_link <- conmat_bp %>% 
...
# system(paste0(infomap_wd,"/Infomap ",infomap_wd,"OBIS.net ",infomap_wd,"test --clu --bftree --tree --map -i 'link-list'"),intern=T)
# 
# grid_boxes_tree_bp <- read.table(paste0(infomap_wd,"test/OBIS.tree")) %>% 
#     mutate(ID=paste0("gb",V3)) %>% 
#     filter(ID %in% unique(unlist(usable$gridID))) %>% 
#     mutate(nlevels=lengths(strsplit(x=as.character(V1),split=":"))-1,
#            cell=gsub(".+:","",V1),
...
#     st_cast("MULTILINESTRING")
# 
# if(max(grid_boxes_tree_bp$nlevels,na.rm=T)>=2){
#     grid_lvl2 <- grid_boxes_tree_bp %>%
#         filter(nlevels>=2) %>% 
#         mutate(lvl1=paste(lvl1,lvl2)) %>% 
#         group_by(lvl1) %>%
#         summarize(geometry=st_union(geometry)) %>% 
#         grid_difference(.,grid_lvl1,grid_boxes_tree_bp) %>%
#         st_cast("MULTILINESTRING") %>%
#         transmute(order="2",
#                   lvl=2,
#                   geometry=geometry)
# }
# 
# if(max(grid_boxes_tree_bp$nlevels,na.rm=T)>=3){
#     grid_lvl3 <- grid_boxes_tree_bp %>%
#         filter(nlevels>=3) %>% 
#         mutate(lvl1=paste(lvl1,lvl2,lvl3)) %>% 
#         group_by(lvl1) %>%
#         summarize(geometry=st_union(geometry)) %>% 
#         grid_difference(.,grid_lvl1,grid_boxes_tree_bp) %>%
#         grid_difference(.,grid_lvl2,grid_boxes_tree_bp) %>%
#         st_cast("MULTILINESTRING") %>%
#         transmute(order="3",
#                   lvl=3,
#                   geometry=geometry)
...
#     grid_lvls <- rbind(grid_lvl1,grid_lvl2,grid_lvl3) 
#     
# } else if(max(grid_boxes_tree_bp$nlevels,na.rm=T)>=2){
#     grid_lvls <- rbind(grid_lvl1,grid_lvl2) 
# } else {
#     grid_lvls <- grid_lvl1
# }
...
# ### plot ###
# #limits
# xlim <- bbox(as(grid_boxes_tree_bp,"Spatial"))[c(1,3)]+c(-40000,+40000)
# ylim <- bbox(as(grid_boxes_tree_bp,"Spatial"))[c(2,4)]+c(-40000,+40000)
# 
# #colors
# n <- ceiling((length(unique(grid_boxes_tree_bp$community))-1)/4)
# colorpal <- c(rbind(rainbow(n,s=0.25),rainbow(n,s=0.5),rainbow(n,s=0.75),rainbow(n,s=1)))
# 
# #plot order
# 
# lvls <- starts_with('lvl',vars=names(grid_boxes_tree_bp))
# grid_boxes_tree_bp[,lvls] <- grid_boxes_tree_bp[,lvls] %>%
#     lapply(as.numeric) %>%
#     data.frame() %>%
#     select(-geometry) %>% 
#     lapply(as.numeric) 
# 
# if(max(grid_boxes_tree_bp$nlevels,na.rm=T)>=3){
#     grid_boxes_tree_bp <-  grid_boxes_tree_bp[with(grid_boxes_tree_bp,order(lvl1,lvl2,lvl3)),]
# } else if(max(grid_boxes_tree_bp$nlevels,na.rm=T)>=2){
#     grid_boxes_tree_bp <-  grid_boxes_tree_bp[with(grid_boxes_tree_bp,order(lvl1,lvl2)),]
# } else {
#     grid_boxes_tree_bp <-  grid_boxes_tree_bp[with(grid_boxes_tree_bp,order(lvl1)),]
# }
# grid_boxes_tree_bp$community <- factor(grid_boxes_tree_bp$community,ordered=T,levels=unique(grid_boxes_tree_bp$community))
# 
# 
# ggplot()+
#     geom_sf(data=coast,colour="transparent",fill='grey') +
#     geom_sf(data=grid_boxes_tree_bp,aes(fill=community),colour="transparent") +
#     geom_sf(data=grid_lvls,fill='transparent',aes(linetype=order,size=order,color=order)) +
#     # geom_sf(data=OBIS[OBIS$species %in% usable,]) +
#     scale_fill_manual("Community",values=colorpal) +
#     scale_linetype_manual("Order",values=c('1'='solid','2'='dashed','3'='dotted')) +
...
# 
# 
# plot(unlist(conmat),unlist(conmat_bp))
# cor(unlist(conmat),unlist(conmat_bp))
